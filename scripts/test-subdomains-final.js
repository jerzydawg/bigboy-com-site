  ...
  try {
    ...
    for (const testCity of testCities) {
      ...
      const { data: stateData, error: stateError } = await supabase
        .from('states')
        .select('id, name')
        .eq('abbreviation', testCity.state)
        .single();

      if (stateError) {
        console.error(`❌ Error finding state ${testCity.state}:`, stateError);
        continue;
      }

      // Then get the city with that state
      ...
      if (cityError) {
        console.error(`❌ Error querying ${testCity.name}:`, cityError);
        continue;
      }
